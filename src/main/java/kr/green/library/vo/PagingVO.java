...
public class PagingVO<T> {
	...
	public PagingVO(int currentPage, int pageSize, int blockSize, int totalCount, String type, String keyword) {
	...
	public PagingVO(int currentPage, int pageSize, int blockSize, int totalCount, String isbn) {
		this.currentPage = currentPage;
		this.pageSize = pageSize;
		this.blockSize = blockSize;
		this.totalCount = totalCount;
	...
	private void calc() {
		...
		if(pageSize<1) pageSize=5; // 1페이지당 글 수가 1보다 적으면 10을 가지자
	...
	public String getPageList() {
		...
		if(startPage>1) {
			...
			message += "<a class='page-link' href='#' onclick='SendPost(\"?\",{\"p\":\""+ (startPage-1) +"\",\"s\":\""+pageSize+"\",\"b\":\""+blockSize+"\",\"type\":\""+type+"\",\"keyword\":\""+keyword+"\",\"isbn\":\""+isbn+"\"})' aria-label='Previous'>";
		...
		for(int i=startPage;i<=endPage;i++) {
			if(i==currentPage) {
				...
				message += "<li class='page-item'><a class='page-link' href='#' onclick='SendPost(\"?\",{\"p\":\""+ (i) +"\",\"s\":\""+pageSize+"\",\"b\":\""+blockSize+"\",\"type\":\""+type+"\",\"keyword\":\""+keyword+"\",\"isbn\":\""+isbn+"\"}), method = get'>" + i + "</a></li>";
			}
		}
		// <%-- 다음 : 마지막 페이지가 전체페이지보다 적다면 다음이 있다 --%>
		if(endPage<totalPage) {
			message += "<li class='page-item'>";
			message += "<a class='page-link' href='#' onclick='SendPost(\"?\",{\"p\":\""+ (endPage+1) +"\",\"s\":\""+pageSize+"\",\"b\":\""+blockSize+"\",\"type\":\""+type+"\",\"keyword\":\""+keyword+"\",\"isbn\":\""+isbn+"\"})' aria-label='Next'>";
	...
	public String toString() {
		return "PagingVO [list=" + list + ", currentPage=" + currentPage + ", pageSize=" + pageSize + ", blockSize="
				+ blockSize + ", type=" + type + ", keyword=" + keyword + ", totalCount=" + totalCount + ", totalPage="
				+ totalPage + ", startNo=" + startNo + ", endNo=" + endNo + ", startPage=" + startPage + ", endPage="
				+ endPage + "]";
	}
